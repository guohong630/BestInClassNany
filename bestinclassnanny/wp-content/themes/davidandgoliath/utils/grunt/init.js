module.exports = function(grunt) {
	...
	return {
		...
		concat: {
			...
			dist: {
				src: [
					...
					'<%= pkg.jsLocation %>plugins/vamtam/jquery.wpvequalheight.js',
					'<%= pkg.jsLocation %>plugins/vamtam/jquery.rawcontenthandler.js',
					'<%= pkg.jsLocation %>plugins/vamtam/jquery.vamtam.slider.js',
					'<%= pkg.jsLocation %>plugins/vamtam/jquery.validator.js',
					'<%= pkg.jsLocation %>plugins/vamtam/jquery.jail.js',
					'<%= pkg.jsLocation %>plugins/vamtam/jquery.vamtam.scrollable.js',
		...
		compress: {
			'push-menu': {
				options: {
					archive: 'vamtam/plugins/vamtam-push-menu.zip',
					mode: 'zip',
					pretty: true
				},
				files: [{
			...
			'love-it': {
				options: {
					archive: 'vamtam/plugins/vamtam-love-it.zip',
					mode: 'zip',
					pretty: true
				},
				files: [{
			...
			layerslider: {
				options: {
					archive: 'vamtam/plugins/layerslider.zip',
					mode: 'zip',
					pretty: true
				},
				files: [{
					...
					src: [
						'layerslider/**/*',
						'!layerslider/sampleslider/**/*.jpg',
						'!layerslider/sampleslider/**/*.gif',
						'!layerslider/sampleslider/**/*.png',
					],
					cwd: 'vamtam/plugins/'
				}]
			},
		...
		parallel: {
			...
			makepot: {
				...
				tasks: [{
					cmd: 'php',
					args: [path.join('/tmp', 'wp-devel', 'tools', 'i18n', 'makepot.php'), 'wp-theme', basedir, path.join(basedir, 'languages', theme_name+'.pot')]
				}]
			},
		},
		clean: {
			...
			'post-copy': {
				src: [
					'build/**/vamtam/plugins/layerslider/',
					'build/**/vamtam/plugins/vamtam-push-menu/',
					'build/**/node_modules',
					'build/**/desktop.ini',
