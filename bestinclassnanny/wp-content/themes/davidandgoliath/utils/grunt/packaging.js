module.exports = function(grunt) {
	...
	grunt.registerTask('download-sidebars-options-layerslider', function() {
		var done = this.async();

		var parts = [
			['sidebars', 'sidebars', 'sidebars'],
			['default-options.php', 'default-options.php', 'options'],
			['layerslider', 'layerslider/sample_sliders.txt', 'exported'],
		], pi = -1;
	...
	grunt.registerMultiTask('add-textdomain', function() {
		...
		for(var fi in files) {
			grunt.file.write(files[fi],
				grunt.file.read(files[fi]).replace(/,\s*(['"])wpv\1/g, ", '"+theme_name+"'").replace("load_theme_textdomain('wpv'", "load_theme_textdomain('"+theme_name+"'")
			);
		}
	});
