...
if(!WpvTemplates::has_header_slider() && empty($content) && empty($min_height) && !WpvFancyPortfolio::has('disabled')) return;
...
?>
<header id="header-middle" class="row <?php echo $fullwidth ? 'fullwidth' : 'normal' ?> <?php echo $type ?>" style="<?php echo $style ?>">
	<?php
	/*
	 * some pages may not have a slider enabled, check for that
	 */
	if( WpvTemplates::has_header_slider() ):
		$slider_engine = 'layerslider';
		?>
