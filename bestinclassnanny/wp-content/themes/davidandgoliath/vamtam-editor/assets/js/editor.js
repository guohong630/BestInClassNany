...
(function($, undefined) {
	...
	$(function() {
		...
		if (ED.container.length === 1) {
			...
			postform.bind('submit.vamtameditor', function(e) {
	...
	var ED = {
		...
		get_editor_html: function(refresh) {
			...
			$.ajax({
				...
				data: {
					...
					content: switchEditors._wp_Nop($('#content').val())
				},
		...
		dragdrop: {
			...
			refresh: function() {
				...
				$('.wpv_sortable:not(.ui-draggable)', ED.container).draggable({
					appendTo: ED.content,
					handle: '> .controls',
					scroll: true,
					zIndex: 1000,
				...
				ED.dragdrop.empty_notice();
				$('.inner-sortable:not(.ui-droppable)', ED.container).droppable({
					tolerance: 'pointer',
					greedy: true,
		...
		tinymce: {
			init: function(element) {
				...
				wrap.find('.switch-tmce').click(function() {
					wrap.removeClass('html-active').addClass('tmce-active');
					element.val(switchEditors.wpautop(element.val()));
					tinyMCE.execCommand("mceAddControl", true, id);
					tinyMCE.activeEditor = tinymce.getInstanceById(id);
					wpActiveEditor = id;

					$('#wp-' + id + '-wrap').mousedown(function(e) {
						e.stopPropagation();
					});
				}).click();

				wrap.find('.switch-html').click(function() {
					wrap.removeClass('tmce-active').addClass('html-active');
					tinyMCE.execCommand("mceRemoveControl", true, id);

					tinyMCE.activeEditor = tinymce.getInstanceById('content');
					wpActiveEditor = 'content';
					element.val(switchEditors._wp_Nop(element.val()));
				}).click();

				wrap.find('.switch-tmce').click();
			},
		...
		edit_form: {
			show: function(element) {
				...
				$.ajax({
					...
					success: function(response) {
						$("#post-preview, #publish, #content-tmce, #content-html").hide();
						...
						$(">.wpv-ed-param-holder", element).each(function() {
							...
							if ($(this).is(".text, .textarea") && $(this).is('div, h1,h2,h3,h4,h5,h6, span, i, b, strong')) {
								...
								new_value = '';
								param.val($(this).is('textarea,input') ? $(this).val() : $(this).html());
			...
			save: function() {
				...
				ED.form.find('[name]').filter('input, textarea, select').each(function() {
					...
					if ($(this).hasClass("tinymce-textarea")) {
						...
						new_value = ED.form.find('[name="' + name + '"]:checked').val();
					} else {
						new_value = $(this).val();
			...
			before_save_cancel: function() {
				$("#post-preview, #publish, #content-tmce, #content-html").show(); // show main publish button
		...
		generate_shortcodes: function(root, level) {
			...
			columns.each(function() {
				...
				element.children('.wpv-ed-param-holder:not(.html-content, .noattr)').each(function() {
					...
					if ($(this).hasClass('multiselect')) param_name = param_name.replace(/\[\]/, '');
				...
				var wrap_before = '',
					wrap_after = '',
					column_title = element.find('> .wpv-ed-param-holder[name="column_title"]').val(),
					column_title_type = element.find('> .wpv-ed-param-holder[name="column_title_type"]').val(),
					column_divider = element.find('> .wpv-ed-param-holder[name="column_divider"]').val(),
					column_animation = element.find('> .wpv-ed-param-holder[name="column_animation"]').val();
		...
		get_icons_selector: function() {
			if(!('the_icons_selector' in this))
				this.the_icons_selector = ED.content.parent().find('.wpv-config-icons-selector');

			return this.the_icons_selector;
		},
			...
			services: {
				init: function() {
					ED.add_callback('init-expandable-services', function() {
						...
						content = content.html().split('[split]');
				...
				init: function() {
					...
					$('body').on('click', '.tab-add', function(e) {
						...
						li.before('<li>\
								...
								<a class="tab-icon-selector no-icon" title="Change Icon" data-icon-name="">&nbsp;</a>\
							</li>');
					...
					$('body').on('click', '.tab-icon-selector', function(e) {
						var selector = ED.get_icons_selector(),
							self = $(this);
						...
						selector.find(':radio').bind('change.wpv-editor-icons', function() {
							...
							selector.find(':radio').unbind('change.wpv-editor-icons');
