...
class WPV_Editor_AJAX {
	...
	private function markup_handler($id, $attributes=null, $content='') {
		...
		?>
			<div id="wpved-<?php echo $id.'-'.uniqid() ?>" class="wpv_ed_column wpv_sortable inner-sortable column column-<?php echo preg_replace('/column-(\d)(\d)/', '$1-$2', $id) ?> expandable" data-basename="column">
				<?php echo $this->getControls('size name add edit clone delete handle', preg_replace('/column-(\d)(\d)/', '$1/$2', $id), __('Column', 'davidandgoliath')) ?>
		...
		<?php else: ?>
			...
			<div id="wpved-<?php echo $id.'-'.uniqid() ?>" class="wpved-<?php echo "$id $accepting $class"; ?> wpv_sortable column <?php echo "$size_class $expandable"?>" <?php echo $this->getCallbacks($id) ?> data-basename="<?php echo $id ?>">
				<?php echo $this->getControls($controls, $size, $wpv_sc[$id]['name']) ?>
	...
	private function getControls($controls, $size_str, $name='') {
		$output = '<div class="controls">';


		$controls_html = array(
			'name' => "<span class='column-name'>$name</span>",
			'size' => "<div class='column-size-wrapper'>
							<div class='column-increase-decrease'><a class='column-increase icon-plus' href='#' title='".__('Increase width', 'davidandgoliath')."'></a> <a class='column-decrease icon-minus' href='#' title='".__('Decrease width', 'davidandgoliath')."'></a></div>
							<span class='column-size'>$size_str</span>
						</div>",
			// 'add' => " <a class='column-add icon-plus' href='#' title='".__('Insert element', 'davidandgoliath')."'></a> ",
			'clone' => " <a class='column-clone icon-copy' href='#' title='".__('Clone', 'davidandgoliath')."'></a> ",
			'edit' => ' <a class="column-edit icon-edit" href="#" title="'.__('Edit shortcode properties', 'davidandgoliath').'"></a>',
			'delete' => ' <a class="column-remove icon-remove" href="#" title="'.__('Remove shortcode', 'davidandgoliath').'"></a>',
			'handle' => '<div class="handlediv" title="'.__('Click to toggle', 'davidandgoliath').'"><br /></div>',
		);
	...
	private function formatParam($param, $value) {
		...
		if(isset($type)) {
			$attr = "class='wpv-ed-param-holder $id $type $class' name='$id'";
	...
	private function get_special_markup($id, $content) {
		...
		switch($id) {
				...
				foreach($sub_contents as $i=>$text): ?>
					...
					<div>
						<h3 class="title-wrapper clearfix">
							...
							<a class="accordion-remove icon-remove" title="<?php _e('Remove', 'davidandgoliath') ?>"></a>
							<a class="accordion-clone icon-copy" title="<?php _e('Clone', 'davidandgoliath') ?>"></a>
						</h3>
			...
			case 'tabs':
				...
				?>
					<li>
						...
						<a class="tab-remove icon-remove" title="<?php esc_attr_e('Remove', 'davidandgoliath') ?>"></a>
						<a class="tab-clone icon-copy" title="<?php esc_attr_e('Clone', 'davidandgoliath') ?>"></a>
						<a class="tab-icon-selector <?php echo wpv_get_icon_type($p['icon']) ?> <?php if(empty($p['icon'])) echo 'no-icon' ?>" data-icon-name="<?php echo $p['icon'] ?>" title="<?php esc_attr_e('Change Icon', 'davidandgoliath') ?>"><?php wpv_icon($p['icon']) ?></a>
					</li>
				<?php endforeach;
			...
			case 'services_expandable':
				echo "<div class='inner-content'>$content</div>";
			break;
	...
	private function do_parse($content) {
		...
		$content = wpv_fix_br(wpv_fix_shortcodes($content));
