...
class WPV_Editor_Parser {
			...
			if($this->content[$pos] == '[') {
				...
				if($this->content[$pos+1] === '/') {
					...
					if($type != $this->ptr->type) {
						...
						throw new Exception("<!-- wpv editor error --><div class='wpv-editor-error'><span style='color: red'>Improperly closed shortcodes. Expected <strong>[/{$this->ptr->type}]</strong>, but found <strong>[/{$type}]</strong> on position ". ($pos - strlen($type) - 2 ."</span><br><br>Context: $context</div>"));
					}
	...
	private function ignore_whitespace($pos) {
		while(empty($this->content[++$pos]) && $pos < $this->input_size);
