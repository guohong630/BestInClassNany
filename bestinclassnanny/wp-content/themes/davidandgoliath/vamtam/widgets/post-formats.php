...
class WPV_Widget_Post_Formats extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array( 
			...
			'description' => __('A list or dropdown of Post Formats', 'davidandgoliath')
		);
		$this->WP_Widget('WPV-post-formats', __('Vamtam - Post Formats', 'davidandgoliath'), $widget_ops);
	}

	function widget( $args, $instance ) {
		extract( $args );

		$title = apply_filters('widget_title', empty( $instance['title'] ) ? __('Post Formats', 'davidandgoliath') : $instance['title'], $instance, $this->id_base);
