 ...
 */
if ( class_exists( 'WP_Importer' ) ) {
class WPV_Import extends WP_Importer {
	...
	function import_start( $file ) {
		...
		if(isset($import_data['frontpage']) && $import_data['frontpage']) {
			update_option('page_on_front', $import_data['frontpage']);
...
<?php
	...
	function process_posts() {
		foreach ( $this->posts as $post ) {
			...
			$post_exists = false && post_exists( $post['post_title'], '', $post['post_date'] );
			if ( $post_exists ) {
			...
			if ( isset( $post['postmeta'] ) ) {
				foreach ( $post['postmeta'] as $meta ) {
					...
					if ( $key ) {
						...
						if ( ! $value ) {
							if($key == 'fancy-portfolio-categories') {
								$value = unserialize(stripslashes($meta['value']));
							} else {
...
function wpv_importer_init() {
	/**
	 * WordPress Importer object for registering the import callback
	 * @global WP_Import $wp_import
	 */
	$GLOBALS['wpv_import'] = new WPV_Import();
	register_importer( 'wpv', 'Vamtam', sprintf(__('Import <strong>posts, pages, comments, custom fields, categories, tags and sidebars</strong> from a %s dummy content file.', 'wordpress-importer'), THEME_NAME), array( $GLOBALS['wpv_import'], 'dispatch' ) );
}
add_action( 'admin_init', 'wpv_importer_init' );
