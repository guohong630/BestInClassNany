	...
	if ( (!is_home() && !is_front_page()) || is_paged() ) {
		...
		if ( is_category() ) {
			...
			$parentCat = get_category($thisCat->parent);
      		...
      		echo $before . __('Archive by category', 'davidandgoliath').' "' . single_cat_title('', false) . '"' . $after;
			...
			if ( get_post_type() != 'post' ) {
				...
				echo '<a href="' . wpv_get_option('post-all-items') . '/">' . __('Blog', 'davidandgoliath') . '</a> ';
			...
			echo $before . __('Search results for', 'davidandgoliath').' "' . get_search_query() . '"' . $after;

		} elseif ( is_tag() ) {
			echo $before . __('Posts tagged', 'davidandgoliath').' "' . single_tag_title('', false) . '"' . $after;

		} elseif ( is_author() ) {
			global $author;
			$userdata = get_userdata($author);
			echo $before . __('Articles posted by', 'davidandgoliath').' ' . $userdata->display_name . $after;

		} elseif ( is_404() ) {
			echo $before . __('Error 404', 'davidandgoliath') . $after;
		}
		...
		if ( $paged ) {
			...
			echo ' ('.__('Page', 'davidandgoliath') . ' ' . $paged.')';
