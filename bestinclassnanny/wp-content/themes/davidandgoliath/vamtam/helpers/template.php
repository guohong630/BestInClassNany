...
?>
		<div class="post-full-thumb">
			<span class="thumbnail">
				<?php the_post_thumbnail( 'page-'.WPV_LAYOUT_TYPE) ?>
			</span>
		</div>
<?php
	endif;
...
function theme_version_check() {
	...
	if($last_version < (int)WpvFramework::getVersion()) {
		wpv_update_option('theme_installed_version', (int)WpvFramework::getVersion());
	}
}
...
function theme_body_classes($body_class) {
	...
	$body_class_conditions = array(
		...
		'has-ajax-portfolio' => $ajax_portfolio,
		...
		'sticky-header' => wpv_get_optionb('sticky-header') && !$full_bg_slider && !$ajax_portfolio,
		'no-page-bottom-padding' => wpv_post_meta(null, 'use-page-bottom-padding', true) == 'false',
	);
