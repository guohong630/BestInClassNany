(function($, undefined) {
	...
	$.fn.wpvColorPicker = function() {
		...
		$('[type=color]:not(.wpv-colorpicker)', self).each(function() {
...
(function($, undefined) {
	...
	$(function() {
		...
		if('WPV_ADMIN' in window && WPV_ADMIN.stylesurl) {
			$('#customize-current-theme-link, .hide-if-no-customize').attr('href', WPV_ADMIN.stylesurl);
		}
	...
	$(function() {
		...
		_.each(groups, function(group) {
			...
			if(post_formats.length) {
				...
				post_formats.insertBefore($('#postdivrich')).show();
			}
		});
	});
})(jQuery);