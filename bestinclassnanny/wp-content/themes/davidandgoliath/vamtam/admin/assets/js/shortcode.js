...
(function($, undefined) {
	...
	var shortcode = {
		...
		fields: {
			...
			button: {
				atts: ['id', 'style', 'class', 'align', 'link', 'linkTarget', 'bgColor', 'font', 'icon', 'icon_placement', 'icon_color'],
				content: 'text'
			},
			chart: {
		...
		generate: function() {
			...
			if(type === 'pre_code') {
				var s = shortcode.getVal('pre_code', 'type');
				s = this.def(s === '', 'code', s);

				return '[' + s + ']' + shortcode.getVal('pre_code', 'content') + '[/' + s + ']';
			}
