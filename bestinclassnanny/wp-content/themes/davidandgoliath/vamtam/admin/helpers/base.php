...
function wpv_save_config($options) {
	...
	foreach($options as $option) {
		if(isset($option['id']) && ! empty($option['id'])) {
			...
			$places = array('post', 'page', 'portfolio', 'lightbox', 'product');
...
function wpv_update_generated_css() {
	...
	do_action('wpv_update_generated_css');
}
...
function wpv_validate_install() {
	...
	if( strpos(str_replace(WP_CONTENT_DIR.'/themes/', '', get_template_directory()), '/') !== false ) {
		$wpv_errors[] = __('The theme must be installed in a directory which is a direct child of wp-content/themes/', 'davidandgoliath');
	}

	if(!is_writable(WPV_CACHE_DIR)) {
		$wpv_errors[] = sprintf(__('You must set write permissions (755 or 777) for the cache directory (%s)', 'davidandgoliath'), WPV_CACHE_DIR);
	}

	if( !extension_loaded('gd') || !function_exists('gd_info') ) {
		$wpv_errors[] = __("It seems that your server doesn't have the GD graphic library installed. Please contact your hosting provider, they should be able to assist you with this issue", 'davidandgoliath');
	}
			...
			function wpv_invalid_install() {
				...
				?>
					<div class="updated fade error" style="background: #FEF2F2; border: 1px solid #DFB8BB; color: #666;"><p>
						<?php _e('There were some some errors with your Vamtam theme setup:', 'davidandgoliath')?>
						<ul>
