...
new Vamtam_Updates( array(
	...
	define('LS_PLUGIN_VERSION', '4.6.3');
...
/********************************************************/
	...
	function layerslider_enqueue_content_res() {
		...
		wp_enqueue_script('layerslider_js', $GLOBALS['lsPluginPath'].'js/layerslider.kreaturamedia.jquery.js', array('jquery'), LS_PLUGIN_VERSION, true );
		wp_enqueue_script('jquery_easing', $GLOBALS['lsPluginPath'].'js/jquery-easing-1.3.js', array('jquery'), '1.3.0', true );
		wp_enqueue_script('transit', $GLOBALS['lsPluginPath'].'js/jquerytransit.js', array('jquery'), '0.9.9', true );
		wp_enqueue_script('layerslider_transitions', $GLOBALS['lsPluginPath'].'js/layerslider.transitions.js', array(), LS_PLUGIN_VERSION, true );
		...
		if(file_exists($custom_trs)) {
			wp_enqueue_script('layerslider_custom_transitions', $custom_trs_url, array(), LS_PLUGIN_VERSION, true );
		}
...
function layerslider_import_sample_slider() {
	...
	$path = defined('WPV_SAMPLES_DIR') ? WPV_SAMPLES_DIR . 'layerslider/sample_sliders.txt' : dirname(__FILE__).'/sampleslider/sample_sliders.txt';
	$sample_slider = json_decode(base64_decode(file_get_contents($path)), true);
